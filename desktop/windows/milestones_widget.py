...
class MilestoneDialog(QDialog):
    def __init__(self, dogs: list, event_types: list, record: dict = None, parent=None):
        ...
        self.setWindowTitle("Edit Milestone" if record else "Add Milestone")
        ...
        self._notes2.setMaxLength(60)
        self._notes2.setPlaceholderText("60 chars")
        ...
        layout.addRow("Notes 2", self._notes2)
...
class MilestonesWidget(QWidget):
    def __init__(self):
        ...
        # Dog checkboxes — built dynamically in _build_filters(); placeholders here
        self._dog_checks: dict[int, QCheckBox] = {}   # dog_id → checkbox
        self._all_check = QCheckBox("All")
        self._all_check.setChecked(True)
        self._all_check.stateChanged.connect(self._apply_filter)
        self._filter_toolbar = toolbar   # keep ref so _build_filters can add to it
    ...
    def _apply_filter(self, *_):
        ...
        filtered = [
            ...
            if (r["dog_id"] is None and include_all)
    ...
    def _populate(self, records: list):
        ...
        for i, r in enumerate(records):
            ...
            self._table.setItem(i, 5, QTableWidgetItem(r.get("notes2") or ""))
            w = r.get("weight_lbs")
    ...
    def _add(self):
        dlg = MilestoneDialog(self._dogs, self._event_types, parent=self)
    ...
    def _edit(self):
        ...
        if not record:
            return
        dlg = MilestoneDialog(self._dogs, self._event_types, record=record, parent=self)
        if dlg.exec() == QDialog.DialogCode.Accepted:
            try:
    ...
    def _delete(self):
        ...
        if QMessageBox.question(self, "Confirm Delete", f"Delete this milestone?\n\n{label}") == QMessageBox.StandardButton.Yes:
            try:
